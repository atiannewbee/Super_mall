const SESSION_KEY = 'super-mall-session'
...
export async function request(path, options = {}) {
  ...
  try {
    ...
    throw new ApiError('无法连接商城服务，请确认后端已经启动', { code: 'NETWORK_ERROR', details: { cause: cause?.message } })
  }
  ...
  if (response.status === 401 && typeof window !== 'undefined') {
    ...
    window.dispatchEvent(new CustomEvent('super-mall:unauthorized'))
  }
...
export const api = {
  ...
  post: (path, body, options = {}) => request(path, { ...options, method: 'POST', body: body == null ? undefined : JSON.stringify(body) }),
